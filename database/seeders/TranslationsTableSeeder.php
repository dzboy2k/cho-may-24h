...
class TranslationsTableSeeder extends Seeder
{
    ...
    {
        // Adding translations for 'navs'
        //
        $cat = Category::where('slug', 'category-1')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['navs', 'slug'], $cat->id), 'categoria-1');
            $this->trans('pt', $this->arr(['navs', 'name'], $cat->id), 'Categoria 1');
        }
        $cat = Category::where('slug', 'category-2')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['navs', 'slug'], $cat->id), 'categoria-2');
            $this->trans('pt', $this->arr(['navs', 'name'], $cat->id), 'Categoria 2');
        }
    }
    ...
    private function menusTranslations()
    {
        ...
        $_item = $this->findMenuItem(__('voyager::seeders.menu_items.navs'));
        if ($_item->exists) {
