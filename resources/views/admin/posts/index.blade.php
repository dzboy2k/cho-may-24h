...
@section('title',__('post.post_list'))
@section('header')
    <div class="container-fluid">
        <h1 class="page-title">
            <i class="voyager-documentation"></i>
            {{__('post.post_in_lang')}}
        </h1>
        <a href="{{ route('admin.posts.create.form') }}" class="btn btn-success btn-add-new"
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-body">
                        <form method="get" class="form-search" action="{{route('admin.posts.search')}}">
                            <div id="search-input">
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                @foreach($posts as $post)
                                    <tr>
                                        ...
                                        <td>
                                            <div
                                                class="text-center">{{ mb_strlen( $post->title ) > 200 ? mb_substr($post->title, 0, 100) . ' ...' : $post->title }}</div>
                                        </td>
                                        <td>
                                            <div
                                                class="text-center">{{ mb_strlen( $post->slug ) > 200 ? mb_substr($post->slug, 0, 100) . ' ...' : $post->slug }}</div>
                                        </td>
                                        <td>
                                            <div
                                        ...
                                        <td>
                                            <div class="text-center">
                                                <img src="{{asset(@$post->images[0]->path)}}"
                                                     alt="{{@$post->images[0]->alt}}" width="60">
                                            </div>
                                        </td>
                                        <td>
                                            <div
                                                class="text-center">{{ mb_strlen( $post->release_date ) > 200 ? mb_substr($post->release_date, 0, 200) . ' ...' : $post->release_date }}</div>
                                        </td>
